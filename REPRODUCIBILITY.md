...
Use the same environment family as the original experiments. The primary environment files are `repro/environment-lock-dl.yml`, `repro/requirements-lock-dl.txt`, `DL_env_export.yml`, and `DL_pip_freeze.txt`; the local release check used `PYTHON_BIN=/home/user/miniconda3/envs/DL/bin/python`.
