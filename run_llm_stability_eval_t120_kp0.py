...
OUT = PROJECT / 'llm_stability_eval_t120_kp0'
OUT.mkdir(parents=True, exist_ok=True)

summary = {'runs': []}
for i in range(1, 4):
    run_id = f't120_kp0_r{i}'
    ...
    with log_file.open('w', encoding='utf-8') as lf:
        try:
            p = subprocess.run([PY, '-u', 'generate_llm_payloads.py'], cwd=str(PROJECT), env=env, stdout=lf, stderr=subprocess.STDOUT, timeout=420)
            rc = int(p.returncode)
            timeout_hit = False
    ...
    summary['runs'].append({
        ...
        'proxy_error_count': txt.count('Unable to connect to proxy') + txt.count('ProxyError'),
        'read_timeout_count': txt.count('Read timed out') + txt.count('Request timed out'),
    })
